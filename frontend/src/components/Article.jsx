    ...
    <div className={`min-h-screen ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
         ...
         <div className="flex items-center justify-between">
           <div className="flex items-center space-x-2 sm:space-x-4">
             ...
             <h1 className={`text-lg sm:text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Article</h1>
           ...
           <div className="flex items-center space-x-2 sm:space-x-4">
             <div className="hidden sm:flex items-center space-x-2">
               <input
                 type="text"
                 placeholder="Search"
                 className={`px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent w-32 lg:w-64 text-sm ${
                   isDarkMode 
                     ? 'border-gray-600 bg-gray-700 text-white placeholder-gray-400' 
                     : 'border-gray-300 bg-white text-gray-900 placeholder-gray-500'
                 }`}
               />
               <button className="px-4 py-2 bg-orange-600 text-white font-medium rounded-md hover:bg-orange-700 transition-colors text-sm">
                 SEARCH
               </button>
             </div>
             <button
         ...
         <div className="flex-1 p-4 sm:p-6">
           <article className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-md overflow-hidden max-w-4xl mx-auto`}>
                         {/* Article Image */}
             ...
             <div className="p-4 sm:p-6">
               ...
               <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 space-y-3 sm:space-y-0">
                 <div className="flex items-center space-x-3">
                   <img
                     src={getAvatarUrl(article?.author?.avatar) || "https://images.unsplash.com/photo-1494790108755-2616b612b786?w=50&h=50&fit=crop&crop=face"}
                 ...
                 <button className="px-4 py-2 bg-orange-600 text-white text-sm font-medium rounded-md hover:bg-orange-700 transition-colors self-start sm:self-auto">
                   Follow
                 </button>
               </div>
               ...
               <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between pt-6 border-t border-gray-200 space-y-2 sm:space-y-0">
                 ...
                 <span className="text-gray-500 text-sm">{article?.createdAt ? new Date(article.createdAt).toLocaleDateString() : "3 Days Ago"}</span>
               </div>
            </div>
          </article>
        </div>
                 ...
                 {/* Comments Sidebar */}
         <div className={`w-full lg:w-96 border-t lg:border-l p-4 sm:p-6 ${
           isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'
         }`}>
           <h2 className={`text-lg sm:text-xl font-bold mb-4 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
             ...
             <button
               ...
               className={`px-3 py-2 rounded-md text-xs sm:text-sm font-medium transition-colors ${
                 commentSort === 'top'
                   ...
                   : isDarkMode
                     ? 'bg-gray-700 text-orange-400 border border-orange-600'
          ...
          <div className="space-y-4 mb-6">
           ...
           {isAuthenticated ? (
             <form onSubmit={handleSubmitComment} className="flex space-x-2">
               ...
               <button
                 type="submit"
                 className="p-2 bg-orange-600 text-white rounded-md hover:bg-orange-700 transition-colors flex-shrink-0"
               >
                 <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className={`text-center py-4 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
             ...
             {/* Floating Chat Icon */}
       <div className="fixed right-4 sm:right-8 bottom-4 transform -translate-y-1/2">
         <button 
